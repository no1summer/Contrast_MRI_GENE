...
T2 IDPs with T2 UDIP traits (MoCoV2 and ViT)
...
trait_dir1 = Path("/data484_4/txia2/gwas_practice/individual_phenos/contrast_learning_mocov2_T2_5std")
trait_dir2 = Path("/data484_4/txia2/gwas_practice/individual_phenos/vit_t2_fixed")
ukb_all_path = Path("/data5/Ziqian/UKBB/UKB_data/UKB_all.csv")
output_dir = Path("/data484_4/txia2/mocov2/IDP_PhenoWAS/idp_omnibus_ftest_T2_IDP_T2_UDIP")
output_dir.mkdir(parents=True, exist_ok=True)

MIN_N = 10000
N_PCS = 40

print("=== Omnibus Partial F-test: T2 IDPs with T2 UDIP Traits ===\n")
print(f"Trait dataset 1 (MoCoV2 T2 UDIP): {trait_dir1}")
print(f"Trait dataset 2 (ViT T2 UDIP): {trait_dir2}\n")

# ---------------- Load T2-related IDP IDs ----------------
print("Loading T2-related IDP IDs...")
idp_id_xlsx = Path("/data484_4/txia2/mocov2/IDP_PhenoWAS/IDP_ID.xlsx")
df_idp_ids = pd.read_excel(idp_id_xlsx)
t2_mask_desc = df_idp_ids['IDP description'].str.contains('T2', case=False, na=False)
t2_mask_short = df_idp_ids['IDP short name'].str.contains('T2', case=False, na=False)
t2_combined = t2_mask_desc | t2_mask_short
t2_ukb_ids = [str(int(x)) for x in df_idp_ids[t2_combined]['UKB ID'].tolist() if pd.notna(x)]
print(f"Found {len(t2_ukb_ids)} T2-related UKB IDs: {t2_ukb_ids}")
...
# Filter to T2-related IDPs
t2_idp_cols = [col for col in all_idp_cols if any(ukb_id in col for ukb_id in t2_ukb_ids)]
print(f"Found {len(t2_idp_cols)} T2-related IDP columns in merged data")

# Filter IDP fields to those with > MIN_N samples
print(f"Filtering T2 IDP fields to those with > {MIN_N} samples...")
idp_sample_counts = idp_df[t2_idp_cols].notna().sum()
idp_cols = idp_sample_counts[idp_sample_counts > MIN_N].index.tolist()
print(f"Found {len(idp_cols)} T2 IDP fields with > {MIN_N} samples")
...
print(f"Dataset 1 (MoCoV2 T2 UDIP) traits loaded: {traits1.shape}")

traits2 = load_traits(trait_dir2)
print(f"Dataset 2 (ViT T2 UDIP) traits loaded: {traits2.shape}")

# Run omnibus F-test for both datasets
results1 = run_omnibus_ftest(traits1, idp_df, covars, idp_cols, covar_cols, "mocov2_T2_UDIP")
results2 = run_omnibus_ftest(traits2, idp_df, covars, idp_cols, covar_cols, "vit_T2_UDIP")
...
if len(res_df) > 0:
    ...
    # Save results
    output_path = output_dir / "idp_omnibus_ftest_T2_IDP_T2_UDIP.csv"
    ...
    color_map = {'mocov2_T2_UDIP': '#3498db', 'vit_T2_UDIP': '#e74c3c'}
    ...
    col_order = ['vit_T2_UDIP', 'mocov2_T2_UDIP']
    ...
    ax1.set_xlabel('T2 IDP Field', fontsize=20, fontweight='bold')
    ax1.set_ylabel('F-statistic', fontsize=20, fontweight='bold')
    ax1.set_title('Omnibus F-statistic by T2 IDP (T2 UDIP Traits)', fontsize=20, fontweight='bold')
    ax1.tick_params(axis='x', rotation=45, labelsize=20)
    ax1.tick_params(axis='y', labelsize=20)
    ax1.legend(['ViT_T2_UDIP', 'MoCoV2_T2_UDIP'], title='Dataset', fontsize=20, title_fontsize=20)
    ...
    ax2.set_xlabel('T2 IDP Field', fontsize=20, fontweight='bold')
    ax2.set_ylabel('-log10(P-value)', fontsize=20, fontweight='bold')
    ax2.set_title('Omnibus P-value by T2 IDP (T2 UDIP Traits)', fontsize=20, fontweight='bold')
    ax2.tick_params(axis='x', rotation=45, labelsize=20)
    ax2.tick_params(axis='y', labelsize=20)
    ax2.legend(['ViT_T2_UDIP', 'MoCoV2_T2_UDIP', 'p=0.05'], title='Dataset', fontsize=20, title_fontsize=20)
    ...
    ax3.set_xlabel('T2 IDP', fontsize=20, fontweight='bold')
    ax3.set_ylabel('Incremental R²', fontsize=20, fontweight='bold')
    ax3.set_title('Incremental R² (T2 UDIP Traits Contribution After Covariates)', fontsize=20, fontweight='bold')
    ax3.tick_params(axis='x', rotation=45, labelsize=20)
    ax3.tick_params(axis='y', labelsize=20)
    ax3.legend(['ViT_T2_UDIP', 'MoCoV2_T2_UDIP'], fontsize=18, loc='upper right')
    ...
    if 'mocov2_T2_UDIP' in pivot_f.columns and 'vit_T2_UDIP' in pivot_f.columns:
        valid_mask = pivot_f.notna().all(axis=1)
        if valid_mask.sum() > 0:
            x = pivot_f.loc[valid_mask, 'mocov2_T2_UDIP']
            y = pivot_f.loc[valid_mask, 'vit_T2_UDIP']
            ...
            ax4.set_xlabel('F-statistic: MoCoV2 T2 UDIP', fontsize=20, fontweight='bold')
            ax4.set_ylabel('F-statistic: ViT T2 UDIP', fontsize=20, fontweight='bold')
            ax4.set_title('F-statistic Comparison (T2 IDP, T2 UDIP Traits)', fontsize=20, fontweight='bold')
...
# ---------------- Plotting from saved CSV ----------------
def plot_from_saved_csv(csv_path=None):
    """Plot figures from saved omnibus F-test results CSV."""
    if csv_path is None:
        csv_path = output_dir / "idp_omnibus_ftest_T2_IDP_T2_UDIP.csv"
    ...
    # Figure 3: Incremental R²
    fig3, ax3 = plt.subplots(figsize=(14, 8))
    pivot_r2[col_order].plot(kind='bar', ax=ax3, width=0.8, color=colors)
    ax3.set_xlabel('T2 IDP Field', fontsize=20, fontweight='bold')
    ax3.set_ylabel('Incremental R²', fontsize=20, fontweight='bold')
    ax3.set_title('Incremental R² (T2 UDIP Traits Contribution After Covariates)', fontsize=20, fontweight='bold')
    ax3.tick_params(axis='x', rotation=45, labelsize=16)
    ax3.tick_params(axis='y', labelsize=16)
    ax3.legend(['ViT_T2_UDIP', 'MoCoV2_T2_UDIP'], title='Dataset', fontsize=18, title_fontsize=18)
    ax3.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    fig3.savefig(output_dir / "omnibus_incremental_r2_by_idp.png", dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'omnibus_incremental_r2_by_idp.png'}")
...
def plot_line_from_saved_csv(csv_path=None):
    ...
    print(f"\n=== Plotting Line Plots from saved CSV: {csv_path} ===")
    res_df = pd.read_csv(csv_path)
    ...
    for col, color in zip(col_order, colors):
        label = 'ViT_T2' if col == 'vit_T2_UDIP' else 'MoCoV2_T2'
    ...
    if 'mocov2_T2_UDIP' in pivot_f.columns and 'vit_T2_UDIP' in pivot_f.columns:
        ...
        if valid_mask.sum() > 0:
            ...
            max_val = max(x.max(), y.max()) * 1.1
            ax4.plot([0, max_val], [0, max_val], 'k--', linewidth=2, label='y=x')
            ax4.set_xlabel('F-statistic: MoCoV2 T2 UDIP', fontsize=16)
            ax4.set_ylabel('F-statistic: ViT T2 UDIP', fontsize=16)
            ax4.set_title('F-statistic Comparison (T2 IDP, T2 UDIP Traits)', fontsize=20, fontweight='bold')

...
output_dir = Path("/data484_4/txia2/mocov2/IDP_PhenoWAS/idp_omnibus_ftest_T2_IDP_T1_UDIP")
output_dir.mkdir(parents=True, exist_ok=True)

MIN_N = 10000
N_PCS = 40

print("=== Omnibus Partial F-test: All Traits vs Each IDP ===\n")
print(f"Trait dataset 1: {trait_dir1}")
print(f"Trait dataset 2: {trait_dir2}\n")
...
print(f"Dataset 1 traits loaded: {traits1.shape}")

traits2 = load_traits(trait_dir2)
print(f"Dataset 2 traits loaded: {traits2.shape}")

# Run omnibus F-test for both datasets
results1 = run_omnibus_ftest(traits1, idp_df, covars, idp_cols, covar_cols, "mocov2_partial_fit_5std")
results2 = run_omnibus_ftest(traits2, idp_df, covars, idp_cols, covar_cols, "vit_t1_fixed")
...
if len(res_df) > 0:
    ...
    # Save results
    output_path = output_dir / "idp_omnibus_ftest_comparison.csv"
    ...
    for dataset in res_df['dataset'].unique():
        ...
        print(f"  Mean R² (traits only): {dataset_df['r2_traits_only'].mean():.4f}")
    ...
    color_map = {'mocov2_partial_fit_5std': '#3498db', 'vit_t1_fixed': '#e74c3c'}
    ...
    col_order = ['vit_t1_fixed', 'mocov2_partial_fit_5std']
    ...
    ax1.set_title('Omnibus F-statistic by IDP', fontsize=20, fontweight='bold')
    ax1.tick_params(axis='x', rotation=45, labelsize=20)
    ax1.tick_params(axis='y', labelsize=20)
    ax1.legend(['ViT_T1', 'MoCoV2_T1'], title='Dataset', fontsize=20, title_fontsize=20)
    ...
    ax2.set_title('Omnibus P-value by IDP', fontsize=20, fontweight='bold')
    ax2.tick_params(axis='x', rotation=45, labelsize=20)
    ax2.tick_params(axis='y', labelsize=20)
    ax2.legend(['ViT_T1', 'MoCoV2_T1', 'p=0.05'], title='Dataset', fontsize=16, title_fontsize=18)
    ...
    ax3.set_title('Incremental R² (Traits Contribution After Covariates)', fontsize=20, fontweight='bold')
    ax3.tick_params(axis='x', rotation=45, labelsize=20)
    ax3.tick_params(axis='y', labelsize=20)
    ax3.legend(['ViT_T1', 'MoCoV2_T1'], title='Dataset', fontsize=20, title_fontsize=20)
    ...
    if 'mocov2_partial_fit_5std' in pivot_f.columns and 'vit_t1_fixed' in pivot_f.columns:
        valid_mask = pivot_f.notna().all(axis=1)
        if valid_mask.sum() > 0:
            x = pivot_f.loc[valid_mask, 'mocov2_partial_fit_5std']
            y = pivot_f.loc[valid_mask, 'vit_t1_fixed']
            ...
            ax4.set_xlabel('F-statistic: MoCoV2', fontsize=20, fontweight='bold')
            ax4.set_ylabel('F-statistic: ViT', fontsize=20, fontweight='bold')
            ax4.set_title('F-statistic Comparison', fontsize=20, fontweight='bold')
...
def plot_from_saved_csv(csv_path=None):
    ...
    if csv_path is None:
        csv_path = output_dir / "idp_omnibus_ftest_comparison.csv"
    ...
    ax2.tick_params(axis='x', rotation=45, labelsize=20)
    ax2.tick_params(axis='y', labelsize=20)
    ax2.legend(['ViT_T1', 'MoCoV2_T1', 'p=0.05'], title='Dataset', fontsize=20, title_fontsize=20)
    ax2.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    fig2.savefig(output_dir / "omnibus_pvalue_by_idp.png", dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'omnibus_pvalue_by_idp.png'}")
    ...
    ax3.legend(['ViT_T1', 'MoCoV2_T1'], title='Dataset', fontsize=18, title_fontsize=18)
    ax3.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    fig3.savefig(output_dir / "omnibus_incremental_r2_by_idp.png", dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'omnibus_incremental_r2_by_idp.png'}")
...
def plot_line_from_saved_csv(csv_path=None):
    ...
    for col, color in zip(col_order, colors):
        label = 'ViT_T1' if col == 'vit_t1_fixed' else 'MoCoV2_T1'
    ...
    ax3.set_ylabel('Incremental R²', fontsize=20, fontweight='bold')
    ax3.set_title('Incremental R² (T1 Traits Contribution After Covariates)', fontsize=20, fontweight='bold')
    ...
    if 'mocov2_partial_fit_5std' in pivot_f.columns and 'vit_t1_fixed' in pivot_f.columns:
        ...
        if valid_mask.sum() > 0:
            ...
            max_val = max(x.max(), y.max()) * 1.1
            ax4.plot([0, max_val], [0, max_val], 'k--', linewidth=2, label='y=x')
            ax4.set_xlabel('F-statistic: MoCoV2', fontsize=16)
            ax4.set_ylabel('F-statistic: ViT', fontsize=16)
            ax4.set_title('F-statistic Comparison', fontsize=20, fontweight='bold')

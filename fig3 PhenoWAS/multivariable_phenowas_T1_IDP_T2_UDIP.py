...
T1 IDPs with T2 traits (MoCoV2 and ViT)
...
output_dir = Path("/data484_4/txia2/mocov2/IDP_PhenoWAS/idp_omnibus_ftest_T1_IDP_T2_UDIP")
output_dir.mkdir(parents=True, exist_ok=True)

MIN_N = 10000
N_PCS = 40

print("=== Omnibus Partial F-test: T1 IDPs with T2 Traits ===\n")
print(f"Trait dataset 1 (MoCoV2 T2): {trait_dir1}")
print(f"Trait dataset 2 (ViT T2): {trait_dir2}\n")
...
print(f"Dataset 1 (MoCoV2 T2) traits loaded: {traits1.shape}")

traits2 = load_traits(trait_dir2)
print(f"Dataset 2 (ViT T2) traits loaded: {traits2.shape}")

# Run omnibus F-test for both datasets
results1 = run_omnibus_ftest(traits1, idp_df, covars, idp_cols, covar_cols, "mocov2_T2")
results2 = run_omnibus_ftest(traits2, idp_df, covars, idp_cols, covar_cols, "vit_T2")
...
if len(res_df) > 0:
    ...
    # Save results
    output_path = output_dir / "idp_omnibus_ftest_T1_IDP_T2_traits.csv"
    ...
    color_map = {'mocov2_T2': '#3498db', 'vit_T2': '#e74c3c'}
    ...
    ax1.set_title('Omnibus F-statistic by T1 IDP (T2 Traits)', fontsize=20, fontweight='bold')
    ax1.tick_params(axis='x', rotation=45, labelsize=20)
    ax1.tick_params(axis='y', labelsize=20)
    ax1.legend(['ViT_T2', 'MoCoV2_T2'], title='Dataset', fontsize=20, title_fontsize=20)
    ...
    ax2.set_title('Omnibus P-value by T1 IDP (T2 Traits)', fontsize=20, fontweight='bold')
    ax2.tick_params(axis='x', rotation=45, labelsize=20)
    ax2.tick_params(axis='y', labelsize=20)
    ax2.legend(['ViT_T2', 'MoCoV2_T2', 'p=0.05'], title='Dataset', fontsize=20, title_fontsize=20)
    ...
    ax3.legend(['ViT_T2', 'MoCoV2_T2'], fontsize=18, loc='upper right')
    ...
    if 'mocov2_T2' in pivot_f.columns and 'vit_T2' in pivot_f.columns:
        valid_mask = pivot_f.notna().all(axis=1)
        if valid_mask.sum() > 0:
            x = pivot_f.loc[valid_mask, 'mocov2_T2']
            y = pivot_f.loc[valid_mask, 'vit_T2']
            ...
            ax4.set_title('F-statistic Comparison (T1 IDP, T2 Traits)', fontsize=20, fontweight='bold')
...
def plot_line_from_saved_csv(csv_path=None):
    ...
    if csv_path is None:
        csv_path = output_dir / "idp_omnibus_ftest_T1_IDP_T2_traits.csv"
    ...
    for col, color in zip(col_order, colors):
        label = 'ViT_T2' if col == 'vit_T2' else 'MoCoV2_T2'
    ...
    ax3.set_xlabel('T1 IDP', fontsize=20, fontweight='bold')
    ax3.set_ylabel('Incremental R²', fontsize=20, fontweight='bold')
    ax3.set_title('Incremental R² (T2 Traits Contribution After Covariates)', fontsize=20, fontweight='bold')

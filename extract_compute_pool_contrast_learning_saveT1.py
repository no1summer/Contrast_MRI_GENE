#!/usr/bin/env python
"""Extract compute_pool features from a trained contrastive learning ViT model (T1 version).

Usage:
    python extract_compute_pool_contrast_learning_T1.py \
...
class InferenceWrapper(torch.nn.Module):
    ...
    def forward(self, x_T1, x_T2):
        """
        Extract latent representations from encoder_q for T1 images.
        Returns the latent before pooling/projection, which serves as compute_pool.
        """
        # Use encoder_q to encode T1 images
        y_T1 = x_T1
        latent, batch_mask, non_zero_patch = self.model.encoder_q(x_T1, y_T1)
...
def extract_compute_pool(checkpoint_path, data_file_T1, data_file_T2, output_path, batch_size=32, num_workers=4):
    ...
    representations from encoder_q for T1 images, and saves them as features.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Extract compute_pool from a trained contrastive learning UDIP-ViT model (T1 version).')
    ...
    parser.add_argument('--output_dir', type=str, default="compute_pool_features_contrast_learning_T1", help='Path to save the output CSV files.')
